...
from app.models import LetterOfCredit, Document, AuditRecord, Discrepancy, DEFAULT_RULE_CONTENT
...
class AuditEngine:
    def __init__(self, lc: LetterOfCredit, documents: List[Document], presentation_date: date,
        ...
        if rule_version and hasattr(rule_version, 'rules') and rule_version.rules:
            ...
            self.rules = dict(DEFAULT_RULE_CONTENT)

        self.amount_tolerance = self.rules.get("amount_tolerance", 0.01)
        self.date_tolerance_days = self.rules.get("date_tolerance_days", 0)
        self.name_case_sensitive = self.rules.get("name_case_sensitive", False)
        self.enabled_categories = self.rules.get("enabled_categories", [
            "completeness", "amount", "date", "party", "goods", "transport", "special"
        ])
        self.severity_overrides = self.rules.get("severity_overrides", {})

    def _apply_severity_override(self, category: str, check_key: str, default_severity: str) -> str:
        category_overrides = self.severity_overrides.get(category, {})
        if check_key in category_overrides:
            override = category_overrides[check_key]
            if override in ("critical", "minor"):
                return override
    ...
    def _names_similar(self, name1: str, name2: str) -> Tuple[bool, bool]:
        ...
        if n1 == n2:
            return True, False
        if not self.name_case_sensitive:
    ...
    def _amount_less_than(self, a: float, b: float) -> bool:
        return (b - a) > self.amount_tolerance
    ...
    def check_completeness(self):
        for req in self.lc.document_requirements:
            ...
            if not submitted:
                severity = self._apply_severity_override("completeness", "missing_document", "critical")
                self.result.add(
            ...
            if submitted.original_copies_submitted < req.original_copies:
                default_severity = "critical" if req.original_copies > 0 and submitted.original_copies_submitted == 0 else "minor"
                severity = self._apply_severity_override("completeness", "original_copies_insufficient", default_severity)
                self.result.add(
                    "completeness", severity, doc_type,
            ...
            if submitted.copy_copies_submitted < req.copy_copies:
                severity = self._apply_severity_override("completeness", "copy_copies_insufficient", "minor")
                self.result.add(
                    "completeness", severity, doc_type,
    ...
    def check_amount(self):
        ...
        if invoice:
            ...
            if self._amount_less_than(self.lc.amount, invoice_amount):
                severity = self._apply_severity_override("amount", "invoice_exceeds_lc", "critical")
                self.result.add(
                    "amount", severity, "invoice",
        ...
        if insurance and invoice:
            ...
            if self._amount_less_than(insurance_amount, min_insurance):
                default_severity = "critical" if has_110_clause else "minor"
                severity = self._apply_severity_override("amount", "insurance_below_110", default_severity)
                self.result.add(
            ...
            if insurance.get("currency") and invoice.get("currency"):
                if insurance["currency"] != invoice["currency"]:
                    severity = self._apply_severity_override("amount", "insurance_currency_mismatch", "minor")
                    self.result.add(
                        "amount", severity, "insurance",
    ...
    def check_date(self):
        ...
        tolerance = timedelta(days=self.date_tolerance_days)
        ...
        if invoice:
            ...
            if inv_date_str:
                ...
                if inv_date > self.presentation_date + tolerance:
                    severity = self._apply_severity_override("date", "invoice_date_after_presentation", "critical")
                    self.result.add(
                        "date", severity, "invoice",
        ...
        if bl:
            ...
            if ship_date_str:
                ...
                if shipment_date > self.lc.latest_shipment_date + tolerance:
                    severity = self._apply_severity_override("date", "shipment_after_latest", "critical")
                    self.result.add(
                        "date", severity, "bill_of_lading",
                        f"提单装船日期{shipment_date}晚于信用证最迟装运日期{self.lc.latest_shipment_date}",
                        "最迟装运日期条款"
                    )

        if self.presentation_date > self.lc.latest_presentation_date + tolerance:
            severity = self._apply_severity_override("date", "presentation_after_latest", "critical")
            self.result.add(
                "date", severity, None,
        ...
        if shipment_date:
            if self.presentation_date > shipment_date + timedelta(days=21) + tolerance:
                severity = self._apply_severity_override("date", "presentation_after_21_days", "critical")
                self.result.add(
                    "date", severity, None,
            ...
            if insurance:
                ...
                if ins_date_str:
                    ...
                    if ins_date > shipment_date + tolerance:
                        severity = self._apply_severity_override("date", "insurance_after_shipment", "critical")
                        self.result.add(
                            "date", severity, "insurance",
                            f"保险投保日期{ins_date}晚于装船日期{shipment_date}",
                            "UCP600第28条e款"
                        )

        if self.presentation_date > self.lc.expiry_date + tolerance:
            severity = self._apply_severity_override("date", "presentation_after_expiry", "critical")
            self.result.add(
                "date", severity, None,
    ...
    def check_party(self):
        ...
        if invoice:
            ...
            if not match:
                severity = self._apply_severity_override("party", "beneficiary_mismatch", "critical")
                self.result.add(
                    "party", severity, "invoice",
                    f"发票受益人'{inv_beneficiary}'与信用证受益人'{self.lc.beneficiary_name}'不一致",
                    "信用证受益人条款"
                )
            elif has_diff:
                severity = self._apply_severity_override("party", "beneficiary_case_diff", "minor")
                self.result.add(
                    "party", severity, "invoice",
            ...
            if not match:
                severity = self._apply_severity_override("party", "applicant_mismatch", "minor")
                self.result.add(
                    "party", severity, "invoice",
                ...
                severity = self._apply_severity_override("party", "applicant_case_diff", "minor")
                self.result.add(
                    "party", severity, "invoice",
        ...
        if bl:
            ...
            if not shipper_otherwise and not match:
                severity = self._apply_severity_override("party", "shipper_beneficiary_mismatch", "minor")
                self.result.add(
                    "party", severity, "bill_of_lading",
                    f"提单托运人'{bl_shipper}'与信用证受益人'{self.lc.beneficiary_name}'不一致",
                    "UCP600默认规则"
                )
            elif not shipper_otherwise and has_diff:
                severity = self._apply_severity_override("party", "shipper_beneficiary_case_diff", "minor")
                self.result.add(
                    "party", severity, "bill_of_lading",
    ...
    def check_goods(self):
        ...
        if invoice:
            ...
            if inv_goods_norm and inv_goods_norm != lc_goods_norm:
                ...
                if not match_space:
                    severity = self._apply_severity_override("goods", "goods_description_mismatch", "critical")
                    self.result.add(
                        "goods", severity, "invoice",
                        f"发票货物描述与信用证不匹配: 发票'{inv_goods_norm}' vs 信用证'{lc_goods_norm}'",
                        "信用证货物描述条款"
                    )
                else:
                    severity = self._apply_severity_override("goods", "goods_description_case_diff", "minor")
                    self.result.add(
                        "goods", severity, "invoice",
                ...
                if doc_content:
                    ...
                    if doc_goods and main_product_name:
                        ...
                        if total_check > 0 and contradiction_count == total_check:
                            severity = self._apply_severity_override("goods", "goods_contradiction", "minor")
                            self.result.add(
                                "goods", severity, doc_type,
    ...
    def check_transport(self):
        ...
        if bl_port_load.lower() != lc_port_load.lower():
            severity = self._apply_severity_override("transport", "port_of_loading_mismatch", "critical")
            self.result.add(
                "transport", severity, "bill_of_lading",
        ...
        if bl_port_disch.lower() != lc_port_disch.lower():
            severity = self._apply_severity_override("transport", "port_of_discharge_mismatch", "critical")
            self.result.add(
                "transport", severity, "bill_of_lading",
        ...
        if not self.lc.transshipment_allowed:
            ...
            if has_trans_marker:
                severity = self._apply_severity_override("transport", "transshipment_violation", "critical")
                self.result.add(
                    "transport", severity, "bill_of_lading",
                    "提单显示转运标记，但信用证禁止转运",
                    "信用证转运条款"
            ...
            if invoice:
                ...
                if inv_qty > 0 and bl_packages > 0:
                    if bl_packages < inv_qty * 0.90:
                        severity = self._apply_severity_override("transport", "partial_shipment_violation", "critical")
                        self.result.add(
                            "transport", severity, "bill_of_lading",
        ...
        for idx, term in enumerate(self.lc.additional_terms):
            ...
            if "指示抬头" in term or "to order" in term_lower:
                ...
                if bl:
                    ...
                    if "to order" not in consignee.lower() and "指示" not in consignee:
                        severity = self._apply_severity_override("special", "consignee_not_to_order", "critical")
                        self.result.add(
                            "special", severity, "bill_of_lading",
            ...
            if "空白背书" in term or "blank endorsed" in term_lower:
                ...
                if bl:
                    ...
                    if "blank" not in endorsement.lower() and "空白" not in endorsement:
                        severity = self._apply_severity_override("special", "no_blank_endorsement", "minor")
                        self.result.add(
                            "special", severity, "bill_of_lading",
                            "提单未按要求做成空白背书",
                            f"附加条款第{idx + 1}条: {term}"
            ...
            if "运费预付" in term or "freight prepaid" in term_lower:
                ...
                if bl:
                    ...
                    if "prepaid" not in freight.lower() and "预付" not in freight:
                        severity = self._apply_severity_override("special", "freight_not_prepaid", "critical")
                        self.result.add(
                            "special", severity, "bill_of_lading",
            ...
            if "运费到付" in term or "freight collect" in term_lower:
                ...
                if bl:
                    ...
                    if "collect" not in freight.lower() and "到付" not in freight:
                        severity = self._apply_severity_override("special", "freight_not_collect", "critical")
                        self.result.add(
                            "special", severity, "bill_of_lading",
            ...
            if "清洁提单" in term or "clean" in term_lower:
                ...
                if bl:
                    ...
                    if not clean or ("瑕疵" in remarks or "damaged" in remarks.lower()):
                        severity = self._apply_severity_override("special", "unclean_bol", "critical")
                        self.result.add(
                            "special", severity, "bill_of_lading",
                            f"提单含有不清洁批注: {remarks}",
                            f"附加条款第{idx + 1}条: {term}"
            ...
            if "一切险" in term or "all risks" in term_lower:
                ...
                if insurance:
                    ...
                    if "all risks" not in risks.lower() and "一切险" not in risks:
                        severity = self._apply_severity_override("special", "no_all_risks", "minor")
                        self.result.add(
                            "special", severity, "insurance",
                            f"保险险别'{risks}'未包含一切险",
                            f"附加条款第{idx + 1}条: {term}"
                        )
            ...
            if "通知方" in term:
                ...
                if bl:
                    ...
                    if not notify:
                        severity = self._apply_severity_override("special", "no_notify_party", "minor")
                        self.result.add(
                            "special", severity, "bill_of_lading",
                            "提单未填写通知方信息",
                            f"附加条款第{idx + 1}条: {term}"
    ...
    def run_audit(self) -> Tuple[str, List[Dict[str, Any]]]:
        category_check_map = {
            ...
            "party": self.check_party,
            "goods": self.check_goods,
            "transport": self.check_transport,
            "special": self.check_special_terms,
        }

        for category, check_func in category_check_map.items():
