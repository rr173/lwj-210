...
def test_refusal_waiver_module():
    ...
    try:
        ...
        print("\n" + "-" * 40)
        print("测试9: 关键不符点未豁免时不能waived_accept")
        print("-" * 40)

        if critical_items:
            try:
                crud.bank_register_final_result(db, d3_number, models.REFUSAL_STATUS_WAIVED_ACCEPT, confirmed_by="银行经理")
        ...
        print("\n" + "-" * 40)
        print("测试12: 超时处置后银行直接确认豁免接受，冻结自动解除")
        print("-" * 40)
        ...
        print("\n" + "-" * 40)
        print("测试16: 银行登记return_documents")
        print("-" * 40)
